...
class fully_train:
    def __init__(self, net, conf):
        ...
        ################# SAVE FILES #################
        self.name_to_save = self.conf.name_arch
        self.name_arch_temp = self.conf.name_arch+"_"+str(0).zfill(3)
        num=0
        if os.path.exists(self.conf.out_path + self.name_arch_temp+'.pth'):
            while(os.path.exists(self.conf.out_path + self.name_arch_temp+'.pth')):
                self.name_arch_temp = self.conf.name_arch+"_"+str(num).zfill(3)
                num += 1
        self.name_arch = self.name_arch_temp
        self.num = str(num).zfill(3) # To save a file with the experiment number
        open(self.conf.out_path + self.name_arch + '.pth', 'w').close()
        print(colored("future file :   " + self.conf.out_path + self.name_arch + '.pth','yellow'))
        
    def save_model(self):
        torch.save({'arch': self.name_arch,
                    ...
                    , self.conf.out_path + self.name_arch+'.pth')
    ...
    def CloseLogger(self, text='', interrump=""):
        dicts={'best train acc':self.train_acc,
                ...
                'epoch':self.best_epoch, 
                'experiment': self.conf.dataset+"-"+self.num}
        file={'train acc ' + self.name_arch  :  self.train_accs,
                'test acc ' + self.name_arch :  self.test_accs,
                'train loss '+ self.name_arch:  self.train_losses,
                'test loss '+ self.name_arch :  self.test_losses,
                }
        rlog.save_log(dicts=dicts, file=file, ShortName_in=interrump+self.name_to_save, ExperimentName_in=self.name_arch+text, GeneralComments_in='')
...
if __name__ == '__main__':
    ...
    net = nets.ChooseNet(conf.architecture, conf.n_classes, pretrained=conf)  # Choose the netural network in this function Nerworks/Xception.py -- 
