...
Image.MAX_IMAGE_PIXELS = 800000000000
import torch
import torchvision
...
if True:
    ...
    matrix.load_state_dict(torch.load('models/r41.pth'))
    vgg.cuda(); 
    matrix.cuda()
    features = []
    means = []
    with torch.no_grad():
        for iteration, (styleV, t) in enumerate(style_loader_):
            sF = vgg(styleV.cuda())
            sF = sF["r41"].clone() # remove r41 in other cases
            sb,sc,sh,sw = sF.size()
            sFF = sF.view(sb,sc,-1)
